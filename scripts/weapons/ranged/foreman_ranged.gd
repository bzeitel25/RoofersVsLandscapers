...
func _ready() -> void:
	...
	# Visual: chunky grey/yellow industrial gun shape
	var body = MeshInstance3D.new()
	var b_mesh = BoxMesh.new()
	b_mesh.size = Vector3(0.2, 0.3, 0.6)
	var b_mat = StandardMaterial3D.new()
	b_mat.albedo_color = Color.DARK_GRAY
	...
	t_mesh.size = Vector3(0.22, 0.1, 0.4)
	var t_mat = StandardMaterial3D.new()
	t_mat.albedo_color = Color.YELLOW
	t_mesh.material = t_mat
	trim.mesh = t_mesh
	trim.position = Vector3(0, 0.1, -0.1)
...
func _fire_projectile() -> void:
	...
	s_mesh.bottom_radius = 0.05
	s_mesh.height = 0.15
	var p_mat = StandardMaterial3D.new()
	p_mat.albedo_color = Color.GRAY
